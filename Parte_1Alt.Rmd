...
GA_rosenbrock <- ga(type = TYPE,
                    fitness = function(x){-funcion_rosenbrock(x)},
                    lower = c(-500 , -500, -500), upper = c(500 , 500, 500),
                    maxiter = 2000,
                    run = 200,
                    population = gaControl(TYPE)$population,
                    selection = gaControl(TYPE)$selection,
                    ...
                    pcrossover = 0.9, 
                    pmutation = 0.05, 
                    elitism = base::max(1, round(popSize*0.15)), 
                    seed = 555,
...
```{r show_gif_rosen_3d, echo=FALSE, out.width='100%'}
knitr::include_graphics("GA_Rosenbrock3D.gif")
...
```{r show_gif, echo=FALSE, out.width='100%'}
